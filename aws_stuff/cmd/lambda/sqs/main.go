...
func process(ctx context.Context, proc service.Processor, event events.SQSEvent, log *zap.Logger) error {
	for _, rec := range event.Records {
		if err := proc.HandleSQS(ctx, rec.Body); err != nil {
			log.Error("SQS handle error", zap.Error(err))
		}
	}
	return nil
}
